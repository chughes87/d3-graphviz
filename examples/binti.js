/*
  ...
  * integrate NextJS
  * add search box
  * togglebody of just highlighted node
...
const redirect = (selectedNodes, excludedNodes, includeBody, highlightedNode) =>
  window.location.assign(buildQueryString(selectedNodes, excludedNodes, includeBody, highlightedNode))

const buildQueryString = (selectedNodes, excludedNodes, includeBody, highlightedNode) => '?' +
    Object.entries({
        selectedNodes,
        excludedNodes,
...
const filterData = ({ nodes, edges }, selectedNodes, excludedNodes) => {
    ...
    const relatedNodes = relatedEdges
        .map(e => [e.from, e.to].find(name => !selectedNodes.includes(name) && !excludedNodes.includes(name)))

    return {
        nodes: nodes.filter(n => selectedNodes.includes(n.name) || !!relatedNodes.includes(n.name)),
        edges: relatedEdges
    }
}
...
let graphviz = d3.select("#graph").graphviz({ useWorker: true })
    .on('initEnd', render)
...
function highlightNode(title) {
    highlightedEdges.map(s => s.attr('stroke', 'black').attr('stroke-width', '1px'))
    ...
    const edges = d3.selectAll('.edge').filter(x => x.key.match(new RegExp(`(^${highlightedNode}->|->${highlightedNode}$)`)))
    const path = edges.selectAll('path').attr('stroke', 'red').attr('stroke-width', '5px')
...
function registerEventHandlers() {
    ...
    nodes.filter(x => selectedNodes.includes(x.key)).attr('stroke-width', '2px')
